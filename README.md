...
lib/
  ...
  constants.ts        triggers, experiments, goal, trend window, storage keys
  storage.ts          localStorage read/write helpers
  stats.ts             pure derived-stat functions (gaps, streaks, trends, insights)
  demoData.ts          generates a realistic simulated stretch of days, for demos
```
...
**Important technical honesty:** a browser (and this web app) cannot detect real physical phone pickups. Every pickup here is logged manually — via the button, the `Space` key, or the "Load demo data" simulator. A future native Android/iOS app could use real device unlock events, subject to OS permissions.
...
- A **10-day trend chart** against your daily goal
